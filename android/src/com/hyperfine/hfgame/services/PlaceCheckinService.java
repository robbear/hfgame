...
import com.hyperfine.hfgame.content_providers.QueuedCheckinsContentProvider;
...
public class PlaceCheckinService extends IntentService {
	...
	public void onCreate() {
		if(D)Log.d(TAG, "PlaceCheckinService.onCreate");
		...
		sharedPreferences = getSharedPreferences(Config.PlacesConstants.SHARED_PREFERENCE_FILE, Context.MODE_PRIVATE);
	...
	protected void onHandleIntent(Intent intent) {
		if(D)Log.d(TAG, "PlaceCheckinService.onHandleIntent");
		
		// Retrieve the details for the checkin to perform.
		String reference = intent.getStringExtra(Config.PlacesConstants.EXTRA_KEY_REFERENCE);
		String id = intent.getStringExtra(Config.PlacesConstants.EXTRA_KEY_ID);
		long timeStamp = intent.getLongExtra(Config.PlacesConstants.EXTRA_KEY_TIME_STAMP, 0);
		...
		boolean inBackground = sharedPreferences.getBoolean(Config.PlacesConstants.EXTRA_KEY_IN_BACKGROUND, true);
		...
		if (!isConnected) {
			...
			try {
				...
				if (successfulCheckins.size() > 0) {
					...
					if(D)Log.d(TAG, "Deleted: " + deleteCount);
				}
	...
	protected boolean checkin(long timeStamp, String reference, String id) {
		if (reference != null) {
			try {
				...
				URI uri = new URI(Config.PlacesConstants.PLACES_CHECKIN_URI + Config.PlacesConstants.PLACES_API_KEY);
				...
				if (response.getStatusLine().getReasonPhrase().equals(Config.PlacesConstants.PLACES_CHECKIN_OK_STATUS)) {
					long lastCheckin = sharedPreferences.getLong(Config.PlacesConstants.SP_KEY_LAST_CHECKIN_TIMESTAMP, 0);
					if (timeStamp > lastCheckin) {   
						sharedPreferencesEditor.putLong(Config.PlacesConstants.SP_KEY_LAST_CHECKIN_TIMESTAMP, timeStamp);
						sharedPreferencesEditor.putString(Config.PlacesConstants.SP_KEY_LAST_CHECKIN_ID, id);
						sharedPreferenceSaver.savePreferences(sharedPreferencesEditor, false);
						Intent intent = new Intent(Config.PlacesConstants.NEW_CHECKIN_ACTION);
						intent.putExtra(Config.PlacesConstants.EXTRA_KEY_ID, id);
	...
	protected boolean addToQueue(long timeStamp, String reference, String id) {
		...
		try {
			...
			if(E)Log.e(TAG, "Queuing checkin for " + reference + " failed.");
		}
