...
public class PlaceDetailsUpdateService extends IntentService {
	...
	public void onCreate() {
		if(D)Log.d(TAG, "PlaceDetailsUpdateService.onCreate");
	...
	protected void onHandleIntent(Intent intent) {
		if(D)Log.d(TAG, "PlaceDetailsUpdateService.onHandleIntent");
		...
		boolean forceCache = intent.getBooleanExtra(Config.PlacesConstants.EXTRA_KEY_FORCEREFRESH, false);
		...
		if (!doUpdate) {
			...
			try { 
				...
				if (cursor.moveToFirst()) {
					if (cursor.getLong(cursor.getColumnIndex(PlaceDetailsContentProvider.KEY_LAST_UPDATE_TIME)) > System.currentTimeMillis()-Config.PlacesConstants.MAX_DETAILS_UPDATE_LATENCY) {
						doUpdate = false;
					}
				}
			}
	...
	protected void refreshPlaceDetails(String reference, boolean forceCache) {    
		...
		try {
			...
			String placesFeed = Config.PlacesConstants.PLACES_DETAIL_BASE_URI + reference + Config.PlacesConstants.PLACES_API_KEY;
	...
	protected boolean addPlaceDetail(String id, String name, String vicinity, String types, Location location, String viewport, String icon, String reference, String phone, String address, float rating, String url, boolean forceCache) {    
		...
		try {
			...
			if(E)Log.e(TAG, "Adding Detail for " + name + " failed.");
		}
